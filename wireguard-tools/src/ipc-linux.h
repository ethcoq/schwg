...
again:
	...
	if (!peer) {
		...
		if (dev->flags & WGDEVICE_HAS_MLKEM_PRIVATE_KEY) {
			mnl_attr_put(nlh, WGDEVICE_A_MLKEM_PRIVATE_KEY, sizeof(dev->mlkem_private_key), dev->mlkem_private_key);
		}
		if (dev->flags & WGDEVICE_HAS_MLKEM_PUBLIC_KEY) {
			mnl_attr_put(nlh, WGDEVICE_A_MLKEM_PUBLIC_KEY, sizeof(dev->mlkem_public_key), dev->mlkem_public_key);
		}
	...
	for (peer = peer ? peer : dev->first_peer; peer; peer = peer->next_peer) {
		...
		if (!allowedip) {
			...
			if (peer->flags & WGPEER_HAS_MLKEM_PUBLIC_KEY) {
				if (!mnl_attr_put_check(nlh, SOCKET_BUFFER_SIZE, WGPEER_A_MLKEM_PUBLIC_KEY, sizeof(peer->mlkem_public_key), peer->mlkem_public_key))
					goto toobig_peers;
			}
...
static int parse_peer(const struct nlattr *attr, void *data)
{
	...
	case WGPEER_A_PUBLIC_KEY:
		...
		break;
	case WGPEER_A_MLKEM_PUBLIC_KEY:
		if (mnl_attr_get_payload_len(attr) == sizeof(peer->mlkem_public_key)) {
			memcpy(peer->mlkem_public_key, mnl_attr_get_payload(attr), sizeof(peer->mlkem_public_key));
			peer->flags |= WGPEER_HAS_MLKEM_PUBLIC_KEY;
		}
		break;
...
static int parse_device(const struct nlattr *attr, void *data)
{
	...
	case WGDEVICE_A_PRIVATE_KEY:
		...
		break;
	case WGDEVICE_A_MLKEM_PRIVATE_KEY:
		if (mnl_attr_get_payload_len(attr) == sizeof(device->mlkem_private_key)) {
			memcpy(device->mlkem_private_key, mnl_attr_get_payload(attr), sizeof(device->mlkem_private_key));
	...
	case WGDEVICE_A_PUBLIC_KEY:
		...
		break;
	case WGDEVICE_A_MLKEM_PUBLIC_KEY:
		if (mnl_attr_get_payload_len(attr) == sizeof(device->mlkem_public_key)) {
			memcpy(device->mlkem_public_key, mnl_attr_get_payload(attr), sizeof(device->mlkem_public_key));
