...
static char *key_len_pub(const uint8_t *key, size_t key_len)
{
	static char base64[MLKEM_PUBLIC_KEY_B64];

	key_to_base64_len(base64, key, key_len);
	return base64;
}

static char *key_len_priv(const uint8_t *key, size_t key_len)
{
	static char base64[MLKEM_PRIVATE_KEY_B64];
...
{
	fprintf(stderr, "Usage: %s %s { <interface> | all | interfaces } [public-key | private-key | mlkem-public-key | mlkem-private-key | listen-port | fwmark | peers | preshared-keys | endpoints | allowed-ips | latest-handshakes | transfer | persistent-keepalive | dump]\n", PROG_NAME, COMMAND_NAME);
}
...
static void pretty_print(struct wgdevice *device)
{
	...
	if (device->flags & WGDEVICE_HAS_MLKEM_PUBLIC_KEY) {
		terminal_printf("  " TERMINAL_BOLD "mlkem public key" TERMINAL_RESET ": %s\n", key_len_pub(device->mlkem_public_key, MLKEM_PUBLIC_KEY_LEN));
	}
	if (device->flags & WGDEVICE_HAS_MLKEM_PRIVATE_KEY) {
		terminal_printf("  " TERMINAL_BOLD "mlkem private key" TERMINAL_RESET ": %s\n", key_len_priv(device->mlkem_private_key, MLKEM_PRIVATE_KEY_LEN));
	}
	...
	for_each_wgpeer(device, peer) {
		...
		if (peer->flags & WGPEER_HAS_MLKEM_PUBLIC_KEY)
			terminal_printf("  " TERMINAL_BOLD "mlkem public key" TERMINAL_RESET ": %s\n", key_len_pub(peer->mlkem_public_key, MLKEM_PUBLIC_KEY_LEN));
