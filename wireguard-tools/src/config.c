...
static inline bool parse_key_len(uint8_t *key, const char *value, size_t len)
{
	if (!key_from_base64_len(key, value, len)) {
		fprintf(stderr, "Key is not the correct length or format: `%s'\n", value);
		memset(key, 0, len);
		return false;
	}
...
static bool parse_keyfile_len(uint8_t *key, const char *path, size_t key_len)
{
	FILE *f;
	int c;
	size_t base64_len = ((key_len + 2) / 3) * 4 + 1;
	char dst[base64_len];
	bool ret = false;
	...
	ret = parse_key_len(key, dst, key_len);
...
#define key_match(key) (value = get_value(line, key "="))
	...
	if (ctx->is_device_section) {
		...
		} else if (key_match("MLKEMPrivateKey")) {
			ret = parse_key_len(ctx->device->mlkem_private_key, value, MLKEM_PRIVATE_KEY_LEN);
			if (ret)
				ctx->device->flags |= WGDEVICE_HAS_MLKEM_PRIVATE_KEY;
		} else if (key_match("MLKEMPublicKey")) {
			ret = parse_key_len(ctx->device->mlkem_public_key, value, MLKEM_PUBLIC_KEY_LEN);
			if (ret)
				ctx->device->flags |= WGDEVICE_HAS_MLKEM_PUBLIC_KEY;
		} else
			goto error;
		...
		if (key_match("Endpoint"))
			...
			ret = parse_key_len(ctx->last_peer->mlkem_public_key, value, MLKEM_PUBLIC_KEY_LEN);
			if (ret) {
				ctx->last_peer->flags |= WGPEER_HAS_MLKEM_PUBLIC_KEY;
			}
...
struct wgdevice *config_read_cmd(const char *argv[], int argc)
{
	...
	while (argc > 0) {
		if (!strcmp(argv[0], "listen-port") && argc >= 2 && !peer) {
			...
			argv += 2;
			argc -= 2;
		} else if (!strcmp(argv[0], "mlkem-private-key") && argc >= 2 && !peer) {
			if (!parse_keyfile_len(device->mlkem_private_key, argv[1], MLKEM_PRIVATE_KEY_LEN))
				goto error;
			device->flags |= WGDEVICE_HAS_MLKEM_PRIVATE_KEY;
			argv += 2;
			argc -= 2;
		} else if (!strcmp(argv[0], "mlkem-public-key") && argc >= 2 && !peer) {
			if (!parse_keyfile_len(device->mlkem_public_key, argv[1], MLKEM_PUBLIC_KEY_LEN))
				goto error;
			device->flags |= WGDEVICE_HAS_MLKEM_PUBLIC_KEY;
			argv += 2;
			argc -= 2;
			...
			if (!parse_keyfile_len(peer->mlkem_public_key, argv[1], MLKEM_PUBLIC_KEY_LEN))
				goto error;
			device->flags |= WGPEER_HAS_MLKEM_PUBLIC_KEY;
			argv += 2;
			argc -= 2;
		} else {
