...
static void concat_shared_key(u8 dh_calculation[NOISE_PUBLIC_KEY_LEN], 
					u8 shared_key[MLKEM_PUBLIC_KEY_LEN],
...
{
	u8 dh_calculation[NOISE_PUBLIC_KEY_LEN];
	u8 concat_shk[NOISE_PUBLIC_KEY_LEN + 32];
...
static void concat_ephemeral(u8 ephemeral_src[NOISE_PUBLIC_KEY_LEN], 
					u8 mlkem_ephemeral_src[MLKEM_PUBLIC_KEY_LEN],
...
static void concat_ciphertext(u8 ephemeral_src[NOISE_PUBLIC_KEY_LEN], 
					u8 mlkem_ciphertext_src[MLKEM_CIPHERTEXT_LEN],
...
static void message_ephemeral(u8 ephemeral_dst[NOISE_PUBLIC_KEY_LEN],
			      ...
			      u8 mlkem_ephemeral_dst[MLKEM_PUBLIC_KEY_LEN],
			      const u8 mlkem_ephemeral_src[MLKEM_PUBLIC_KEY_LEN],
			      u8 chaining_key[NOISE_HASH_LEN],
			      u8 hash[NOISE_HASH_LEN])
{
	u8 concat_eph[NOISE_PUBLIC_KEY_LEN + MLKEM_PUBLIC_KEY_LEN];
	if (ephemeral_dst != ephemeral_src)
		memcpy(ephemeral_dst, ephemeral_src, NOISE_PUBLIC_KEY_LEN);
	if (mlkem_ephemeral_dst != mlkem_ephemeral_src)
		memcpy(mlkem_ephemeral_dst, mlkem_ephemeral_src, MLKEM_PUBLIC_KEY_LEN);
	concat_ephemeral(ephemeral_dst, mlkem_ephemeral_dst, concat_eph);
	mix_hash(hash, concat_eph, NOISE_PUBLIC_KEY_LEN + MLKEM_PUBLIC_KEY_LEN);
	kdf(chaining_key, NULL, NULL, concat_eph, NOISE_HASH_LEN, 0, 0,
	    NOISE_PUBLIC_KEY_LEN + MLKEM_PUBLIC_KEY_LEN, chaining_key);
}

static void message_ephemeral_resp(u8 ephemeral_dst[NOISE_PUBLIC_KEY_LEN],
			      const u8 ephemeral_src[NOISE_PUBLIC_KEY_LEN],
			      u8 mlkem_ciphertext_dst[MLKEM_CIPHERTEXT_LEN],
			      const u8 mlkem_ciphertext_src[MLKEM_CIPHERTEXT_LEN],
			      u8 chaining_key[NOISE_HASH_LEN],
			      u8 hash[NOISE_HASH_LEN])
{
	u8 concat_ct[NOISE_PUBLIC_KEY_LEN + MLKEM_CIPHERTEXT_LEN];
	if (ephemeral_dst != ephemeral_src)
		memcpy(ephemeral_dst, ephemeral_src, NOISE_PUBLIC_KEY_LEN);
	if (mlkem_ciphertext_dst != mlkem_ciphertext_src)
		memcpy(mlkem_ciphertext_dst, mlkem_ciphertext_src, MLKEM_CIPHERTEXT_LEN);
	concat_ciphertext(ephemeral_dst, mlkem_ciphertext_dst, concat_ct);
	mix_hash(hash, concat_ct, NOISE_PUBLIC_KEY_LEN + MLKEM_CIPHERTEXT_LEN);
	kdf(chaining_key, NULL, NULL, concat_ct, NOISE_HASH_LEN, 0, 0,
	    NOISE_PUBLIC_KEY_LEN + MLKEM_CIPHERTEXT_LEN, chaining_key);
}
				      ...
				      struct wg_device *wg)
{
	...
	memcpy(handshake->mlkem_ephemeral_public, epq, MLKEM_PUBLIC_KEY_LEN);
				    ...
				    struct wg_device *wg)
{
	...
	u8 mlkem_ciphertext[MLKEM_CIPHERTEXT_LEN];
	...
	memcpy(ephemeral_private, handshake->ephemeral_private,
	       NOISE_PUBLIC_KEY_LEN);
	memcpy(mlkem_ciphertext, src->mlkem_ciphertext,
